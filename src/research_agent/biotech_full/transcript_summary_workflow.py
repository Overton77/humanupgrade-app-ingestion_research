...
async def create_final_client_summary(initial_summary: str, guest_overview: str) -> str:
    ...
    return final_text.strip() if isinstance(final_text, str) else str(final_text).strip()
...
async def store_document_embeddings(
    ...
    episode_transcript_url: str | None = None,
    mongo_episode_id: str | None = None,
) -> List[Document]:
    docs = splitter.create_documents([final_summary_text])

    stable_key = mongo_episode_id or episode_url
    ids = [str(uuid5(NAMESPACE_URL, f"{stable_key}:final_summary:{i}")) for i in range(len(docs))]

    parent_ref = f"episode:{mongo_episode_id}:summaryDetailed" if mongo_episode_id else f"episode_url:{episode_url}"
        ...
        metadata.append(
            {
                ...
                "mongo_episode_id": mongo_episode_id,
                "episode_transcript_url": episode_transcript_url,
                "chunk_index": i,
                "source": "final_summary",

                # Parent pointers (for child->parent reconstruction)
                "parent_type": "episode",
                "parent_mongo_episode_id": mongo_episode_id,
                "parent_field": "summaryDetailed",
                "parent_ref": parent_ref,
            }
        )

    docs = [Document(page_content=d.page_content, metadata=m) for d, m in zip(docs, metadata)]
