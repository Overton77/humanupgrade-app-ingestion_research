...
from research_agent.structured_outputs.candidates_outputs import (CandidateEntity, SeedExtraction, EntitySourceResult, CandidateSourcesConnected)
...
def format_seed_extraction_for_prompt(seed_extraction: Optional[SeedExtraction]) -> Dict[str, str]:
    """
    Formats SeedExtraction into compact, human-readable blocks for prompt injection.
    ...
    def format_single_entity(entity: Optional[CandidateEntity]) -> str:
        if entity is None:
            return "(none)"
        return format_candidate_entity(entity)
    ...
    return {
        "primary_person": format_single_entity(getattr(seed_extraction, "primary_person", None)),
        "primary_organization": format_single_entity(getattr(seed_extraction, "primary_organization", None)),
        "people_candidates": format_entity_list(getattr(seed_extraction, "people_candidates", []) or []),
        "organization_candidates": format_entity_list(getattr(seed_extraction, "organization_candidates", []) or []),
        "product_candidates": format_entity_list(getattr(seed_extraction, "product_candidates", []) or []),
        "compound_candidates": format_entity_list(getattr(seed_extraction, "compound_candidates", []) or []),
        "technology_candidates": format_entity_list(getattr(seed_extraction, "technology_candidates", []) or []),
        "evidence_claim_hooks": format_hooks(getattr(seed_extraction, "evidence_claim_hooks", []) or []),
        "notes": (getattr(seed_extraction, "notes", None) or "(none)").strip() or "(none)",
    }
