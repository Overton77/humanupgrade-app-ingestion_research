...
class OrgIdentityPeopleTechnologySliceModel(BaseModel):
    """
    Slice output for one domain.
    """
...
class ProductsAndCompoundsSliceModel(BaseModel):
    model_config = ConfigDict(extra="allow", str_strip_whitespace=True)
    ...
    technologiesMentioned: List[EntitySourceResultModel] = Field(default_factory=list)

    notes: Optional[str] = None


class ConnectedCandidatesAssemblerInputModel(BaseModel):
    model_config = ConfigDict(extra="allow", str_strip_whitespace=True)
    ...
    orgSlice: OrgIdentityPeopleTechnologySliceModel
    productsSlice: ProductsAndCompoundsSliceModel