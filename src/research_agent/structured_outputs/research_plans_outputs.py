...
class AgentInstancePlanWithoutSources(BaseModel):
    ...
    requires_instance_output: List[InstanceOutputRequirement] = Field(default_factory=list) 
    ...
    notes: Optional[str] = None
InstanceOutputType = Literal["final_report"]  # extend later if needed

class InstanceOutputRequirement(BaseModel):
    """
    Temporary: declare a dependency on another agent instance's output.
    For now, the only supported output is the synthesized final_report.
    """
    from_instance_id: str
    output_type: "InstanceOutputType" = "final_report"
    required: bool = True
    notes: Optional[str] = None
...
class AgentInstancePlanWithSources(BaseModel):
    ...
    starter_inputs: Optional[str] = None
